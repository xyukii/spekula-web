...
class Feedzy_Rss_Feeds_Pro {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Feedzy_Rss_Feeds_Pro_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_filter( 'product_feedzy_license_plan_label', $plugin_admin, 'get_license_plan_label' );
		$this->loader->add_filter( 'feedzy_openai_models', $plugin_admin, 'get_open_ai_models' );
		$this->loader->add_filter( 'feedzy_openai_deprecated_models', $plugin_admin, 'get_deprecated_open_ai_models' );
		...
		add_action(
			'init',
			function () use ( $plugin_admin ) {
				if ( $this->is_free_version_at_most( '5.0.10' ) ) {
					add_action( 'feedzy_general_setting_before', array( $plugin_admin, 'add_license_section' ) );
				}
			
				add_action( 'feedzy_dashboard_license_content', array( $plugin_admin, 'add_license_section' ) );
			}
		);
	}
	...
	/**
	 * Checks if the free version is at most a specific version.
	 * 
	 ...
	 * @since 5.1.0
	 */
	public static function is_free_version_at_most( $version ) {  
		return class_exists( 'Feedzy_Rss_Feeds' ) && version_compare( Feedzy_Rss_Feeds::instance()->get_version(), $version, '<=' );
	}
}
